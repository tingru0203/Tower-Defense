...
Enemy::Enemy(std::string img, float x, float y, float radius, float speed, float hp, int money) :
	Engine::Sprite(img, x, y), speed(speed), hp(hp), money(money), originSpeed(speed) {
...
void Enemy::Update(float deltaTime) {
	if(changeSpeedTicks > 5) {
        changeSpeedTicks = 0;
        speed = originSpeed;
	}
	else if(speed != originSpeed) {
        changeSpeedTicks += deltaTime;
	}
...
void Enemy::ChangeSpeed(float n) {
