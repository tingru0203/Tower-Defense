...
std::vector<std::vector<int>> PlayScene::CalculateBFSDistance() {
	...
	while (!que.empty()) {
		...
		std::vector<Engine::Point> adjacent;
		adjacent.push_back(Engine::Point(0, 1));
		adjacent.push_back(Engine::Point(0, -1));
		adjacent.push_back(Engine::Point(1, 0));
		adjacent.push_back(Engine::Point(-1, 0));
		for(auto a : adjacent) {
            Engine::Point newp = p + a;
            if(newp.x < MapWidth && newp.x >= 0 && newp.y < MapHeight && newp.y >= 0) {
                if(mapState[newp.y][newp.x] == TILE_DIRT && map[newp.y][newp.x] == -1) {
                    que.push(newp);
                    map[newp.y][newp.x] = map[p.y][p.x] + 1;
                }
            }
		}
	}
